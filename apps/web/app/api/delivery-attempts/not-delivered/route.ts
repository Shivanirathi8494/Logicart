  ...
  try {
    ...
    if (!remarks) {
      return NextResponse.json(
        {
          error:
            "Not Delivered reason is required.",
        },
        { status: 400 }
      );
    }
    ...
    if (user.role === "EMPLOYEE") {
      ...
      if (
        ...
        shipment.destination
          .trim()
          .toUpperCase() !== branchCode
      ) {
        return NextResponse.json(
          {
            error:
              "Shipment does not belong to your destination branch.",
          },
          { status: 403 }
        );
      }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            `AWB ${trackingNumber} is currently ${shipment.status}.`,
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          "Unable to record Not Delivered attempt.",
      },
      { status: 500 }
    );
  }
