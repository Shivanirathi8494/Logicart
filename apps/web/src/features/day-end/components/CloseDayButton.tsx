...
export default function CloseDayButton({ summary }: Props) {
  const router = useRouter();

  async function closeDay() {
    const ok = confirm("Close today's business?");

    if (!ok) {
      return;
    }

    const response = await fetch("/api/day-end/close", {
      method: "POST",
    });
  ...
  return (
    <section className="rounded-xl border bg-white p-4 shadow-sm sm:p-6">
      ...
      <div className="space-y-3">
        ...
        {summary.canViewFinancials && <div>✅ Revenue Summary Verified</div>}
      </div>
